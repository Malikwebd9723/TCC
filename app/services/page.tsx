    ...
    <main className={styles.main}>
      <header className={styles.hero}>
        <div className={styles.heroBg}>
          <Image src="/service-gc.png" alt="Our Services" fill style={{ objectFit: "cover" }} priority />
          <div className={styles.heroOverlay}></div>
        </div>
        <div className={`container ${styles.heroContent}`}>
          <h1 className="gold-text">Our Capabilities</h1>
          <p className={styles.subtitle}>Mastery across Residential, Commercial, and Industrial Infrastructure.</p>
        </div>
      </header>

      <section className={`section-padding ${styles.listing}`}>
        <div className="container">
          <div className={styles.servicesGrid}>
            {services.map((service, i) => (
              <div key={i} className={`${styles.serviceBlock} glass`}>
                <div className={styles.serviceImage}>
                    <Image src={service.images[0]} alt={service.title} width={400} height={300} style={{ objectFit: "cover" }} />
                </div>
                <div className={styles.serviceInfo}>
                    <div className={styles.iconHdr}>
                        <div className={styles.iconCol}>{getIcon(service.iconName)}</div>
                        <h2>{service.title}</h2>
                    </div>
                    <p>{service.shortDescription}</p>
                    <Link href={`/services/${service.slug}`} className={`${styles.serviceCta} gold-border`}>
                        View Details &rarr;
                    </Link>
                </div>
              </div>
